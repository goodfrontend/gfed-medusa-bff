...
import type { IntentScore, UserProfile } from '../types/profile.js';

/**
 * Deterministic intent classification.
 * Mirrors the algorithm from the BFF's intent-classifier.ts.
 * Returns scored intents sorted descending by score, normalized to sum to 1.
 */
export function classifyIntent(profile: UserProfile): IntentScore[] {
  ...
  if (ps.score > 0.6 && profile.engagementLevel === 'HIGH' && is.checkoutConversion < 0.3) {
    s.uncertain += 0.3;
  }
